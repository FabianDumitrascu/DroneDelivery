...
int main(int argc, char** argv) {
    ...
    ros::Publisher takeoff_pub_1 = nh.advertise<std_msgs::Empty>("/" + drone_name_1 + "/agiros_pilot/start", 1, true);
    ros::Publisher takeoff_pub_2 = nh.advertise<std_msgs::Empty>("/" + drone_name_2 + "/agiros_pilot/start", 1, true);

    // Ensure that the publishers are valid and the master has been notified of the advertisement
    if (!takeoff_pub_1 || !takeoff_pub_2) {
        ROS_ERROR("Failed to advertise on the takeoff topics!");
        return -1;
    }
