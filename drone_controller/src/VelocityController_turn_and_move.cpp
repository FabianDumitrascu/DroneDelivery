...
public:
    DroneController() : nh("~"), pid1(0.0, 0.0, 0.0, 10.0), pid2(0.0, 0.0, 0.0, 10.0), stop_movement(false), initialYawFound(false), Landing(false), etha_yaw(0.0), etha_translation(0.0), StopTimer(0.0), simulation(false) {
    ...
    void fetchInitialYaw() {
        if (initial_position_falcon1.x != 0.0000000 && initial_position_falcon2.x != 0.0000000) {
            ...
            centerX = startX;
            centerY = startY;
            double initial_yaw_degrees = atan2(dy, dx) * 180.0 / M_PI;
    ...
    void odometryCallback1(const nav_msgs::Odometry::ConstPtr& msg) {
        ...
        if (distance_to_other_drone < 0.5) {
            stop_movement = true;
        }
        if (!stop_movement) {
    ...
    void barCallbacksimulation(const nav_msgs::Odometry::ConstPtr& msg) {
        ...
        double error_yaw = currentYawBar - Mid_yaw_end;
        double error_translation = sqrt(pow(currentPoseBar.position.x - Mid_x, 2) + pow(currentPoseBar.position.y - Mid_y, 2));
    ...
    void barCallback(const geometry_msgs::PoseStamped::ConstPtr& msg) {
        ...
        currentYawBar = QuaternionToYaw(q) * 180.0 / M_PI;
        double error_yaw = 90 + currentYawBar - Mid_yaw_end;
        double error_translation = sqrt(pow(currentPoseBar.position.x - Mid_x, 2) + pow(currentPoseBar.position.y - Mid_y, 2));
        if (fabs(error_yaw) < etha_yaw && error_translation < etha_translation) { 
            StopTimer += 0.01;
            if (StopTimer >= 5.0) {
