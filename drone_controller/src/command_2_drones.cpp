...
int main(int argc, char** argv) {
    ...
    nh.getParam("/BEP/command_node/drone_id1", drone_id1);
    nh.getParam("/BEP/command_node/drone_id2", drone_id2);
    nh.getParam("/BEP/command_node/target_x", target_x);
    nh.getParam("/BEP/command_node/target_y", target_y);
    nh.getParam("/BEP/command_node/target_z", target_z);
    nh.getParam("/BEP/command_node/kp", kp);
    nh.getParam("/BEP/command_node/ki", ki);
    nh.getParam("/BEP/command_node/kd", kd);
    ...
    auto odometryCallback2 = [&vel_pub2, &pid2, &target_position, loop_rate_hz](const nav_msgs::Odometry::ConstPtr& msg) {
        geometry_msgs::TwistStamped velocity_command;
        velocity_command.header.stamp = ros::Time::now();
        velocity_command.twist = pid2.computeControl(target_position, msg->pose.pose.position, 1.0 / loop_rate_hz);
